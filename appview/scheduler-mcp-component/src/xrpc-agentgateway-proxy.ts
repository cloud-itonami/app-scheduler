...
// ../cljs/src/scheduler/app.cljs); backend TypeScript was to be left alone.
// It happened to live inside the now-deleted svelte/ tree, so it is moved
// here rather than deleted, per the same "move, don't delete" treatment used
// for non-Svelte artifacts found under svelte/.
//
...
// fetch handler (mirroring the shape of ../src/app.ts, which proxies to a
// *different* upstream — DISPATCHER_URL, not AGENTGATEWAY_MCP_ROUTER_URL —
// with a different verb surface; see README.md "The two planes expose
// different surfaces") is a product decision this migration does not make.
// It is recorded here, unmodified, so the next reader does not have to
