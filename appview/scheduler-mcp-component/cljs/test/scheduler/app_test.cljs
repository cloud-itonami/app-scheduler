(ns scheduler.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [scheduler.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates the ported +page.svelte `app` object"
    ...
    (is (= "Scheduler Mcp Component" @(rf/subscribe [:title])))
    (is (= "etzhayyim-project-scheduler" @(rf/subscribe [:project])))
    (is (= "scheduler-mcp-component" @(rf/subscribe [:name])))
    (is (= "appview" @(rf/subscribe [:kind])))
    (is (= 2 @(rf/subscribe [:route-count])))
    (is (= ["sc3dul3r.etzhayyim.com/*" "scheduler.etzhayyim.com/*"]
           @(rf/subscribe [:routes])))
    (is (= 8 (count @(rf/subscribe [:vars]))))
    (is (true? @(rf/subscribe [:xrpc?])))
    (is (= "appview/scheduler-mcp-component/cljs/src/scheduler/app.cljs"
