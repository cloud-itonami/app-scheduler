(ns scheduler.app
  "scheduler-mcp-component appview — reagent + re-frame, view built from
  ...
  The Svelte page never called the dispatcher or the kotoba/ reference
  slice (see repo README for the documented drift between those two
  planes) — it was pure static markup describing the surface's own
  metadata, and this port adds no scheduling functionality that was not
  already there.

  Backend TypeScript is unaffected by this migration:
  `../src/app.ts` (the dispatcher Worker) is untouched. The one other
  TypeScript file that lived under the deleted `svelte/` tree —
  ...
  `../src/xrpc-agentgateway-proxy.ts` for its unmodified body and a note on
  why it is not wired to anything right now."
...
;; svelte/src/routes/+page.svelte to here.

(def default-db
  {:title "Scheduler Mcp Component"
   ...
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
          ...
          "APP_UI_TYPE"]
   :xrpc? true
   :relative-path "appview/scheduler-mcp-component/cljs/src/scheduler/app.cljs"})
...
;; source), rebuilt from jp-go-dds primitives instead of the bespoke CSS the
;; Svelte scaffold shipped (`.top` / `.facts` / `.panel` rules) — the visual
;; language is the shared design system's now, not a hand-rolled one, per
;; ADR-2608260900.
